...
const FPS = 30;
// Perform video record and motion detection
let mediaRecorder = null;
let mediaChunks = null;
let processingPreviewIntervalId = null;
// Listen for the processed video frames from the server
socket.on("start_record", function () {
    // Display the processed frame in an image element
    mediaChunks = []
    console.log('--start_record--', mediaRecorder.state)
    (mediaRecorder && mediaRecorder.state !== 'recording') && mediaRecorder.start();
    recordingStartTime = new Date().getTime();
});
// Listen for the processed video frames from the server
socket.on("stop_record", function () {
    console.log('--stop_record--', mediaRecorder.state)
...
async function startMaster(localView, formValues, onStatsReport, onRemoteDataMessage) {
    ...
    function captureFrame(scaleFactor) {
        if(scaleFactor == null){
            scaleFactor = 1;
        }
        var w = localView.videoWidth * scaleFactor;
        var h = localView.videoHeight * scaleFactor;
        var canvas = document.createElement('canvas');
            canvas.width  = w;
            canvas.height = h;
        var ctx = canvas.getContext('2d');
            ctx.drawImage(localView, 0, 0, w, h);
        return canvas;
    }
    ...
    try {
        ...
        function generateRecordingPreview() {
            let mediaBlob = new Blob(mediaChunks, { type: "video/webm;codecs=h264" });
            let mediaBlobUrl = URL.createObjectURL(mediaBlob);
            console.log('store mediaBlobUrl---', mediaBlobUrl)
    
            let downloadButton = document.getElementById("downloadButton");
            downloadButton.href = mediaBlobUrl;
            downloadButton.download = `RecordedVideo-${new Date().getTime()}.mp4`;
        ...
        function processFrame() {
            var type = "image/jpg"
            var scaleFactor = 1
            var frame = captureFrame(scaleFactor)
        ...
        if (formValues.sendVideo || formValues.sendAudio) {
            try {
                ...
                console.log('[MASTER] Video stream # ', master.localStream);
                // START
                ...
                .catch((err) => {
                    console.log("No media device found!", err);
                });
                // END
