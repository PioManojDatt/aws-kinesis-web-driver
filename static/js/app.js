...
$('#master-button').click(async () => {
    ...
    startMaster(cameraPreview, formValues, onStatsReport, event => {
    ...
    if (rtcPeerConnection.connectionState === 'connected') {
        ...
        rtcPeerConnection.getSenders().map(sender => {
            ...
            if (sender.transport) {
                ...
                console.log('Failed to fetch the candidate pair!');
            }
        });
    } else if (rtcPeerConnection.connectionState === 'failed') {
        
        console.log(logPrefix, `Connection to ${remoteClientId || 'peer'} failed!`);
